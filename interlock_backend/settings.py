...
if django_key_spec is not None:
	...
	if (
		"SECRET_KEY" in django_key.__dict__
		and django_key.SECRET_KEY is not None
		and len(django_key.SECRET_KEY) > 0
	):
...
# - interlock_backend/fernet_key.py
# ! If the key changes you will need to re-save your LDAP Bind Password
# ! and all your users will be required to login again
# To MANUALLY generate a new key run this code in python and paste the return value into fernet_key.py
...
if fernet_key_spec is not None:
	...
	if (
		"FERNET_KEY" in fernet_key.__dict__
		and fernet_key.FERNET_KEY is not None
		and len(fernet_key.FERNET_KEY) > 0
	):
		FERNET_KEY = fernet_key.FERNET_KEY
...
FRONT_URL: str
try:
	from interlock_backend.local_django_settings import FRONT_URL
except ImportError:
	pass
