...
class TestDelete:
	...
	def test_successful_multi(mocker: MockerFixture, admin_user_client: APIClient):
		...
		m_validate_record = mocker.Mock(side_effect=[m_valid_record_1, m_valid_record_2])
		m_delete_record = mocker.Mock(side_effect=["record_result_1", "record_result_2"])
	...
	@staticmethod
	def test_successful_multi_raises_malformed_parent(admin_user_client: APIClient):
		m_request_records = 1
