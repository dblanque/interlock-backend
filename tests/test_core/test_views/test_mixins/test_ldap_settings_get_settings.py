...
@pytest.fixture
def fc_setting_override(
	f_default_preset,
) -> Generator[SettingFactoryProtocol, None, None]:
...
class TestGetSettings:
	...
	def test_with_connection_password(
		...
		assert (
			_settings.get(K_LDAP_AUTH_CONNECTION_PASSWORD).get(LOCAL_ATTR_VALUE)
			== "mock_password"
		)
	...
	def test_with_non_decryptable_connection_password(
		...
		mocker.patch(
			"core.views.mixins.ldap_settings.aes_decrypt", side_effect=Exception
		)
		_settings = SettingsViewMixin().get_ldap_settings(f_default_preset.id)
		assert (
			_settings.get(K_LDAP_AUTH_CONNECTION_PASSWORD).get(LOCAL_ATTR_VALUE)
	...
	@pytest.mark.parametrize(
		...
		(
			...
			(
				...
				"CN=TestAdmin,CN=Users,DC=example,DC=org",
			),
