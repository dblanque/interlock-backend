...
class TestLogMixin:
	def test_log_with_invalid_operation_type(
		...
		result = log_mixin.log(
			user=test_user, operation_type="TEST_OPERATION", log_target_class="TestClass"
		)
	...
	def test_log_with_invalid_user_type(self, log_mixin: LogMixin, f_runtime_settings):
		...
		with pytest.raises(TypeError, match="user must be of type int | User"):
			log_mixin.log(
				user="invalid_user", operation_type=LOG_ACTION_READ, log_target_class=LOG_CLASS_USER
			)
	...
	def test_log_with_user_id(self, log_mixin: LogMixin, test_user, f_runtime_settings, mocker):
		...
		m_save: MockType = mocker.patch.object(Log, "save")
		log_mixin.log(
			user=test_user.id, operation_type=LOG_ACTION_UPDATE, log_target_class=LOG_CLASS_USER
		)
	...
	def test_log_rotation_needed(self, log_mixin: LogMixin, test_user, f_runtime_settings, mocker):
		...
		log_mixin.log(
			user=test_user, operation_type=LOG_ACTION_UPDATE, log_target_class=LOG_CLASS_USER
		)
	...
	def test_log_rotation_not_needed(
		...
		# Execute
		mocker.patch.object(Log, "save")
		log_mixin.log(user=test_user, operation_type="TEST_OPERATION", log_target_class="TestClass")
