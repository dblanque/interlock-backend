...
class TestApplicationSecurityGroupViewMixin:
	...
	def test_insert_application_group_invalid_data(
		self, mixin: ApplicationSecurityGroupViewMixin, test_application, application_group_data
	):
		# Corrupt the data
	...
	def test_list_application_groups(
		...
		assert result["application_groups"][0]["id"] == test_application_group.id
		assert (
	...
	def test_retrieve_application_success(
		self, mixin: ApplicationSecurityGroupViewMixin, test_application_group, test_user
	):
		# Test the method
		...
		assert result["application"]["id"] == test_application_group.application.id
		assert result["application"]["name"] == test_application_group.application.name
	...
	def test_update_application_group_success(
		...
		new_user = User.objects.create(
			username="newuser", email="new@example.com", first_name="New", last_name="User"
		)
		...
		# Test the method
		with transaction.atomic():
			mixin.update_application_group(test_application_group.id, update_data)
		...
		assert test_application_group.ldap_objects == update_data["ldap_objects"]
	...
	def test_update_application_group_not_found(
		...
		# Test and verify the exception
		with pytest.raises(ApplicationGroupDoesNotExist):
			mixin.update_application_group(999, application_group_data)  # Non-existent ID
	...
	def test_update_application_group_invalid_data(
		...
		# Test and verify the exception
		with pytest.raises(BadRequest):
			mixin.update_application_group(test_application_group.id, invalid_data)
	...
	def test_change_application_group_status_success(
		...
		# Test the method
		mixin.change_application_group_status(test_application_group.id, {"enabled": False})
	...
	def test_change_application_group_status_not_found(self, mixin):
		# Test and verify the exception
		with pytest.raises(ApplicationGroupDoesNotExist):
			mixin.change_application_group_status(999, {"enabled": False})  # Non-existent ID
	...
	def test_delete_application_group_success(
		...
		assert ApplicationSecurityGroup.objects.count() == 0
		assert not ApplicationSecurityGroup.objects.filter(id=test_application_group.id).exists()
