...
def test_sync_user_relations_admin_user(
	user_dict: dict, in_ldap_admin_group, mocker, f_runtime_settings, f_ldap_connection
):
	...
	sync_user_relations(m_user, m_user_as_ldap_attributes(m_user), connection=f_ldap_connection)
...
def test_sync_user_relations_no_distinguished_name(mocker):
	with pytest.raises(ValueError, match="distinguishedName not present in User LDAP Attributes."):
...
def test_sync_user_relations_dn_as_tuple(
	...
	ldap_attributes["distinguishedName"] = (ldap_attributes["distinguishedName"],)
