...
def test_ldap_filter_not():
	assert (
		LDAPFilter.not_(LDAPFilter.eq("mockAttribute", "mockValue")).to_string()
		== "(!(mockAttribute=mockValue))"
	)
...
def test_parse_nested_ldap_filter_or():
	ldf = LDAPFilter.from_string(
		"(|(mockAttribute1=mockValue1)(&(mockAttribute2=mockValue2)(mockAttribute3=mockValue3)))"
	)
...
@pytest.mark.parametrize(
	"expr",
	(LDAPFilterType.AND, LDAPFilterType.OR),
)
