...
def f_dns_record_a():
	record: DNS_RECORD = new_record(
		RecordTypes.DNS_RECORD_TYPE_A.value, serial=1, ttl=180
	)
...
def f_dns_record_aaaa():
	record: DNS_RECORD = new_record(
		RecordTypes.DNS_RECORD_TYPE_AAAA.value, serial=1, ttl=180
	)
	record["Data"] = DNS_RPC_RECORD_AAAA()
	record["Data"]["ipv6Address"] = socket.inet_pton(
...
def f_dns_record_cname():
	record: DNS_RECORD = new_record(
		RecordTypes.DNS_RECORD_TYPE_CNAME.value, serial=1, ttl=180
	)
...
def f_dns_record_txt():
	record: DNS_RECORD = new_record(
		RecordTypes.DNS_RECORD_TYPE_TXT.value, serial=1, ttl=180
	)
...
def f_dns_record_soa(f_dns_record_soa_values):
	record: DNS_RECORD = new_record(
		RecordTypes.DNS_RECORD_TYPE_SOA.value, serial=1, ttl=180
	)
	data = DNS_RPC_RECORD_SOA()
	for f in ["dwSerialNo", "dwRefresh", "dwRetry", "dwExpire", "dwMinimumTtl"]:
...
class TestDNSRecordStructure:
	...
	def test_dns_record_dunder_str(self, mocker):
		m_dict = {"key": "value"}
		mocker.patch(
			"core.models.structs.ldap_dns_record.record_to_dict",
			return_value=m_dict,
		)
		record = DNS_RECORD()
...
class TestSOARecordHandling:
	...
	def test_soa_record_to_dict(
		self, f_dns_record_soa, f_dns_record_soa_values: dict
	):
...
class TestTSRecordHandling:
	def test_timestamp_conversion(self):
		...
		ts["entombedTime"] = from_datetime(
			datetime(1970, 1, 1)
		)  # Jan 1, 1970 in Windows time
