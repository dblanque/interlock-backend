	...
	def setUp(self):
		...
		with LDAPConnector(force_admin=True) as ldc:
			...
			try:
				...
				if (
					LDAP_AUTH_USER_FIELDS["email"] in data
					and len(data[LDAP_AUTH_USER_FIELDS["email"]]) > 0
				):
					UserViewLDAPMixin.ldap_user_with_email_exists(
						self, email_search=data[LDAP_AUTH_USER_FIELDS["email"]]
					)
