...
class LDAPRecord(LDAPDNS):
	...
	def fetch(self):
		...
		searchFilter = search_filter_add(
			searchFilter, f"distinguishedName={self.distinguishedName}"
		)
		...
		if self.rawEntry["type"] == "searchResEntry":
			...
			for record in self.rawEntry["raw_attributes"]["dnsRecord"]:
				...
				if not record_name.startswith("_") and record_name not in excludeEntries:
	...
	def get_soa_serial(self) -> int:
		...
		if "dwSerialNo" in self.soa:
			...
			if serial_date_obj:
				...
				if not date_changed:
					if len(str(serial)) > 8:
						serial_num = int(str(serial)[8:])
					# Restart counter if serial after datetime is invalid
					elif len(str(serial)) <= 8 or serial_num > 99:
						serial_num = 0
	...
	def create(self, values, debugMode=False):
		...
		# LDAP entry exists
			...
			exists = self.record_exists_in_entry(
				mainField=mainField, mainFieldValue=values[mainField]
			)
			if exists != False:
				logger.error(
					f"{RECORD_MAPPINGS[self.type]['name']} Record already exists in an LDAP Entry (Conflicting value: {values[mainField]})"
				)
				self.connection.unbind()
				data = {
					"typeName": RECORD_MAPPINGS[self.type]["name"],
			...
			if self.type == DNS_RECORD_TYPE_SOA:
				if self.record_soa_exists() == True:
					logger.error(
						f"{RECORD_MAPPINGS[self.type]['name']} Record already exists in an LDAP Entry and must be unique in Zone (Conflicting value: {values[mainField]})"
					)
					logger.error(record_to_dict(dnstool.DNS_RECORD(self.structure.getData())))
					self.connection.unbind()
					data = {
