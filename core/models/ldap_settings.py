...
LDAP_SETTING_MAP = {
	...
	"LDAP_AUTH_USER_FIELDS": TYPE_JSON,
	"LDAP_DIRTREE_OU_FILTER": TYPE_JSON,
	"LDAP_DIRTREE_CN_FILTER": TYPE_JSON,
...
VALIDATORS = {
	TYPE_LDAP_URI: [validate_ldap_uri]
}
FIELD_ARGS = {
	TYPE_LDAP_URI: [models.CharField(max_length=255)]
}
...
for key in LDAP_SETTING_FIELDS.keys():
	LDAP_SETTING_TYPE_CHOICES.append((key, key.upper(),))
...
@add_fields_from_dict(
	LDAP_SETTING_FIELDS,
	validators_dict=VALIDATORS,
	args_pass=FIELD_ARGS
)
class LDAPSetting(BaseSetting):
	...
	preset = models.ForeignKey(
		LDAPPreset,
		verbose_name=_("settings_preset"),
		on_delete=models.CASCADE
	)
	type = models.CharField(
		verbose_name=_("type"),
		choices=LDAP_SETTING_TYPE_CHOICES,
		null=False
	)
